...
class DbDataGen(BaseDbDataGen):
    ...
    def generate_customers(self, num):
        for i in range(num):
            ...
            first_name, last_name, address, phone_number = self.get_customer_data(
                data)
    ...
    def generate_flights_per_company(self, num):
        ...
        for a in airlines:
            for i in range(num):
                ...
                origin_country_id = random.randint(
                    1, self.number_of_countries_in_db)
                destination_country_id = random.randint(
                    1, self.number_of_countries_in_db)
                departure_time = self.fake.date_time_between(
    ...
    def generate_tickets_per_customer(self, num):
        ...
        for c in customers:
            ...
            for f in flights_for_tickets:
                self.repo.add(Tickets(flight_id=f.id,
                                      customer_id=c.id))
