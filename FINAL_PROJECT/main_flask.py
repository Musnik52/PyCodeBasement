...
def signup():
    ...
    if user:
        return Response('User already exists. Please Log in.', status=202, mimetype='application/json')
    else:
        repo.add(Users(username=username, password=generate_password_hash(
            password), email=email, public_id=str(uuid.uuid4()), user_role=3))
...
def login():
    ...
    user = repo.get_by_column_value(
        Users, Users.username, form_data.get('username'))
    if not user:
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            current_user = repo.get_by_column_value(
                Users, Users.public_id, data['public_id'])
        except:
...
def get_or_post_customer():
    ...
    if request.method == 'GET':
        ...
        if len(search_args) == 0:
            Response(json.dumps(customers), status=200,
    ...
    if request.method == 'POST':
        ...
        if repo.get_by_id(Users, new_customer['user_id']):
            return 'Input violates restrictions!'
...
def get_customer_by_id(id):
    ...
    if request.method == 'PUT':
        ...
        if repo.get_by_id(Customers, id) != None:
