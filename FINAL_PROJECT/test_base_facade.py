...
def test_get_flights_by_parameters(base_facade_object):
    sample = base_facade_object.get_flights_by_parameters(
        1, 2, datetime(2022, 1, 1, 10, 10, 10))
    expected = repo.get_by_condition(Flights, lambda query: query.filter(
        Flights.origin_country_id == 1, Flights.destination_country_id == 2, Flights.departure_time == datetime(2022, 1, 1, 10, 10, 10)))
    assert expected == sample


def test_not_get_flights_by_parameters(base_facade_object):
    with pytest.raises(InvalidInput):
        base_facade_object.get_flights_by_parameters(
            '1', 2, datetime(2022, 1, 1, 10, 10, 10))
    with pytest.raises(InvalidInput):
        base_facade_object.get_flights_by_parameters(
            1, '2', datetime(2022, 1, 1, 10, 10, 10))
    ...
    with pytest.raises(FlightNotFound):
        base_facade_object.get_flights_by_parameters(
            3, 1, datetime(2022, 1, 1, 10, 10, 10))
...
def test_get_airline_by_id(base_facade_object):
    assert base_facade_object.get_airline_by_id(
        1) == repo.get_by_id(AirlineCompanies, 1)
...
def test_get_country_by_id(base_facade_object):
    assert base_facade_object.get_country_by_id(
        1) == repo.get_by_id(Countries, 1)
...
def test_create_user(base_facade_object):
    base_facade_object.create_user(Users(
        username='testy4h4v', password='y4h4av5ch', email='testyahav@jb.com', user_role=3))
...
def test_not_create_user(base_facade_object):
    ...
    with pytest.raises(PasswordTooShort):
        base_facade_object.create_user(Users(
            username='testy4h4v', password='123', email='testyahav@jb.com', user_role=3))
