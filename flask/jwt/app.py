...
def signup():
    ...
    if user: return make_response('User already exists. Please Log in.', 202)
    else:
...
def get_all_users(current_user):
    ...
    # convert to json
    output = []
    for user in users: output.append({'public_id': user.public_id, 'name': user.name, 'email': user.email})
